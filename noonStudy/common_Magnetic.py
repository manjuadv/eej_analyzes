...
class IAGA2002(MagneticData, Information):
	...
	def __init__(self, **kwargs):
		...
		self.setReportedOrientation("HDZF")
	...
	def read(self, resource, **kwargs):
		...
		# Close file resource
		if isinstance(resource, str): file.close()
...
'''if __name__ == "__main__":
	ReadObj = IAGA2002()
	ReadObj.read( "CMB20160227pmin.min" )
	ReadObj2 = IAGA2002()
	ReadObj2.read( "CMB20160228pmin.min")
	
	import matplotlib.pyplot as plt
	import numpy as np
	plt.plot(ReadObj.get(ReadObj.datetime_index).append(ReadObj2.get(ReadObj2.datetime_index)), ReadObj.get("x").append(ReadObj2.get("x")))
	plt.plot(np.append(ReadObj.get(ReadObj.datetime_index),ReadObj2.get(ReadObj2.datetime_index)), np.append(ReadObj.get("d"),ReadObj2.get("d")))
	timeSet = np.append(ReadObj.get(ReadObj.datetime_index),ReadObj2.get(ReadObj2.datetime_index))
	dataSet = np.append(ReadObj.get("h"),ReadObj2.get("h"))
